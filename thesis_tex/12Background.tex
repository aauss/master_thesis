...
\begin{align}
  ...
  g_i &= \hat{r} \times G \\
  \forall \boldsymbol{x_i} \text{ repeat } g_i \text{ times: } \boldsymbol{s_i} &= \boldsymbol{x_i} + (\boldsymbol{x_{zi}} - \boldsymbol{x_i}) \times \lambda
\end{align}
...
Finally, a good performance measure is required to evaluate the classifier's learning progress. Accuracy as illustrated before, is not a reliable indicator in a binary imbalanced dataset. The \textbf{Index of Balanced Accuracy} (\GLS{IBA})
