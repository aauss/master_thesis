...
\begin{figure}[h!]
  ...
  \caption{An illustration of a typical pipeline for information extraction in NLP. The pipeline starts with the raw text and undergoes several preprocessing steps (indicated by a rectangle) yielding intermediate output (placed next to the arrows). The output can be a list of tuples of the entity and the corresponding word like \texttt{[(ORG, \textquotesingle Bayer\textquotesingle), (LOC, \textquotesingle Baveria\textquotesingle)]} (adopted from \citep{Bird2009}).}
\label{fig:pipeline}
\end{figure}
...
\subsection{Stop Words}
  ...
  Then we can deduct that this text might be a (news) article and that it is writing about the exit of Great Britain from the European Union.
  ...
  In case we want to find, as an example, adjectives and adverbs in a text for a sentiment analysis then we would not want to discard words that help to localize them since stop words contain information about the grammatical structure of the text.
...
\subsection{Tokenization}\label{tokenization}
  ...
  Tokens not always match how we think about words or sentences which can be shown by the following example.
...
\subsection{(Disease) Named-Entity Recognition}
  ...
  After sentences and words have been tokenized, and POS-tagging was applied, it might be important for some learning algorithms to know the named-entity of the word at hand.
...
\subsection{Corpus}
  A \textbf{corpus}\index{corpus} is a organized collection of (related) text documents that are simple to access. They can be annotated (as required in POS-tagging) or simply preprocessed (e.g. tokenized) \citep{Bengfort2018}.
...
\subsection{Imbalanced Class Problem}
  ...
  Accuracy, as illustrated before, is not a reliable indicator in an imbalanced binary dataset.
...
\subsubsection{Bernoulli Naive Bayes}
  ...
  This Bernoulli variant of the NBC explicitly models the absence and presence of words which is done by a different approach to estimating $p(t_{i}|\mathcal{C}_b)$ (comparison to 2.15).
...
\subsubsection{Convolutional Neural Network}
  While the MLP is only capable of receiving a single vector as an input at a time, the \textbf{convolutional neural network}(\GLS{CNN})\index{convolutional neural network} can process an input matrix.
...
\subsection{Web Scraping}
  The process of automatically extracting content available on the world wide web, i.e., parts of websites is called \textbf{web scraping}\index{web scraping}.
...
\subsubsection{REST}
  ...
  The standard application program interface for this form of communication is the \textbf{Representational State Transfer} (\GLS{REST}), and its methods are GET, POST, PUT, PATCH, and DELETE which are often executed via HTTP.
...
\subsubsection{Ajax}
  ...
  Therefore, it might be necessary to monitor the website's behavior via the developer mode of a web browser and see which GET requests would retrieve data of interest or trigger the Ajax calls programmatically.
