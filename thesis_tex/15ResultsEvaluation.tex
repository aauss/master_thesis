...
\subsection{Data Quality}
  ...
  \begin{table}[h!]
    \centering
    \caption{A performance measure of the transmission of the EDB entries to a controlled vocabulary. The Key Word column stands for the keyword columns of the EDB. The table further shows the number and percentage (out of 557 entries) of valid entries before and after the preprocessing. Note, although the date validity numbers did not change after preprocessing, the dates underwent an important preprocessing step from a string to a timestamp object to allow computation on them.}
    \begin{tabular}{@{}cccc@{}}
      \toprule
...
% \subsection{Evaluation}
  ...
  With only two scrapers, I was able to use half of the entries of the EDB to create a labeled dataset by analyzing the usage frequency (Fig. \ref{fig:netloc}), accessibility, and relevance of the sources (Tab. \ref{table:INIGsources}).
...
\section{Key Information Extraction}
  ...
  To do so, I used the most frequent entity of an entity class, for example, the most frequent disease in one text (Lis. \ref{lst:mostoccure}).
  ...
  Thus, I trained multinomial and Bernoulli NBCs using all sentences of a text containing a specific entity class.
...
\subsection{Results}
  ...
  \end{table}
  %
  Regarding the recall of the \textsl{is key} class and the IBA score, the Bernoulli NBC was performing better with a recall of 0.24 and a IBA of 0.23 while for the count key information extraction. The multinomial NBC had a value of 0.00 for both these measures.
  The Bernoulli NBC was also better performing than the multinomial NBC for the date key information extraction with a recall of 0.11 and a IBA 0.10 while the multinomial NBC had a value of 0.00 for both these measures.
  ...
  Since there were strong expectations which phrases indicated the mentioning of a key entity such as \textit{``\dots confirmed cases\dots"} or \textit{``\dots cases \dots as of\dots"}, I retrieved those token that were strong indicator for the \textsl{is key} label (Tab. \ref{table:important_words}).
...
\subsection{Evaluation}\label{eval_key}
  ...
  When we regard the recall for the \textsl{is key} class and the average IBA score, then for the date and count entity key information extraction, the Bernoulli NBC was the better performing algorithm (\ref{table:keyword_performance_counts} and Tab. \ref{table:keyword_performance_dates}).
  In my opinion, the recall for the \textsl{relevant} class is very important since only correct classifications will decrease the burden for epidemiologists to enter key information into a database.
...
\section{Article Recommendation}
  ...
  Since I intended to also use word embeddings besides the bag-of-words approach to train a classifier, I visualized the word embeddings of the training data pretrained on the Wikipedia and Gigaword Corpus and self-trained embeddings using WHO DON and ProMED articles.
...
\subsection{Results}
  ...
  However, the sizable medical term cluster is not well concentrated.
  %
  \begin{figure}[h!]
    \centering
    \includegraphics[scale=0.6]{t-sne_combined_large.pdf}
  ...
  For comparison, I also plotted the ROC-curves and AUC values of all classifier (Fig. \ref{fig:roc}). The AUC value for the CNN is the highest and for both NBC the worst.
...
\subsection{Evaluation}\label{eval_recommend}
  The pretrained word embeddings do not cover the same amount of technical vocabulary compared to the self-trained embeddings. But on the other hand, the pretrained embeddings have other semantically meaningful clusters like adjectives or names which are missing in the self-trained embeddings.
  It is likely that a more extensive corpus of epidemiological articles would show prominent technical vocabulary cluster and common word clusters at the same time which then would yield a better foundation for training classifier.

  Against expectations, the multinomial and complement NBC had an identical performance, although the complement NBC tackles problems occurring in imbalances datasets specifically seen in Tab. \ref{table:recommender_performance} and Fig \ref{fig:roc}.
  ...
  It appears that the AUC value, although commonly used to measure the performance of classifier trained with imbalanced classes, does not give a good measure for objective of this thesis as the IBA or recall does since the CNN is performing best but has a recall for \textsl{relevant} article of 0.00 (Tab. \ref{table:recommender_performance}).
  ...
  But for now, the support vector machine is preferred due to its good IBA and recall value for the \textsl{relevant} class.
...
\subsection{Results}
  ...
  The output of this evaluation is the extraction of keyword information of this article and a relevance score  (Fig. \ref{fig:t-aussinator}).
...
\subsection{Evaluation}
  ...
  However, the live extraction is rather slow. The processing of one URL takes one second and is noticeably too slow for spontaneous evaluations. This delay is particularly noticeable when extracting several articles as intended by \texttt{Get ProMED articles}.
  ...
  A solution would be a faster CPU.
